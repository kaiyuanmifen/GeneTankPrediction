...
my $ref_path = $RefDirect."23andme_v".$version."_hg".$hg."_ref.txt.gz"; #problem: cannot use ~/Documents/...
...
#open files
my $fh = ($raw_path =~ m/zip$/) ? IO::File->new("unzip -c $raw_path|") : IO::File->new($raw_path);
