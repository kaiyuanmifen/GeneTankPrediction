...
FinalReport$Category="To be determined"
Output=mapply(FinalReport$Phenotype_Name,FUN = function(x){which.min(stringdist(x,PhenoTypeClass$Phenotype, method="jw"))})
OutputValue=mapply(FinalReport$Phenotype_Name,FUN = function(x){min(stringdist(x,PhenoTypeClass$Phenotype, method="jw"))})
FinalReport$Category[OutputValue<=0.3]=PhenoTypeClass$Category[Output][OutputValue<=0.3]
